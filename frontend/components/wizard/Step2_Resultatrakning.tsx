...
const Step2_Resultatrakning = ({ accounts, onAccountChange, onNext, onBack }) => {
  const incomeStatementAccounts = accounts.filter(acc => {
    const accNum = parseInt(acc.account_number, 10);
    return accNum >= 3000 && accNum < 9000;
  });
    ...
    <div>
      ...
      <div className="overflow-x-auto relative shadow-md sm:rounded-lg" style={{ maxHeight: '500px', overflowY: 'auto' }}>
        <table className="w-full text-sm text-left text-gray-500">
          ...
          <tbody>
            {incomeStatementAccounts.map((account, index) => {
